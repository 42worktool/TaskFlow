...
- **현재 멤버 초대는 Redis에 7일간 저장되는 1회용 bearer 링크다.**
  이메일은 링크 전달 수단이며 수락 계정의 이메일과 같을 필요가 없다. 로그인
  후 현재 계정을 확인하고 명시적으로 수락하면 토큰을 먼저 삭제한 뒤 멤버십을
  생성한다. 이후 DB 작업이 실패하면 초대 메일을 다시 보내야 한다. 초대 목록·
  취소·재전송이나 감사 이력이 필요해지면 영속 DB 모델을 추가한다.

- **공개 워크스페이스를 보는 비구성원에게 멤버 이메일을 노출하지 않는다.**
  멤버 이름과 프로필은 공개 보드의 구성원 표시용으로 제공하되, 이메일은
  워크스페이스 구성원에게만 내려 팀원 관리 화면에서 사용한다.
...
- **Toy 프로토타입의 달력은 기존 워크스페이스 리스트 조회를 재사용한다.**
  현재 규모에서는 프론트가 카드 기간을 월 단위로 표시한다. 전용 달력 조회는
  카드 수가 커져 서버 필터링이 필요할 때 추가한다.

- **향후 전용 달력 조회는 월과 겹치는 카드만 반환한다.**
  `GET /workspaces/{workspace_id}/calendar?month=YYYY-MM`을 추가할 때는 카드 기간
  `[start_at, deadline]`이 요청 월의 `[월초, 월말]`과 겹치면 포함하고, 두 날짜가
  모두 `null`인 카드는 제외한다. `month`는 `YYYY-MM` 형식으로 서버에서 검증한다.
