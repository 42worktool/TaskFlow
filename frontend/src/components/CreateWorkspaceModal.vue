...
const emit = defineEmits<{ close: []; created: [Workspace] }>()
...
async function submit() {
  ...
  error.value = ''
  try {
    const ws = await WorkspaceAPI.create(trimmed, isPublic.value)
    emit('created', ws)
    emit('close')
  } catch {
    error.value = '생성에 실패했습니다. 다시 시도해주세요.'
  } finally {
    loading.value = false
  }
}
