// 실시간 연결의 인증과 재연결 복구, 시간 초과, 송신 보호 장치를 검증한다.
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import {
  RealtimeClient,
  RealtimeSendError,
  type RealtimeClientOptions,
} from '../../../src/services/realtime/client'
import {
  REALTIME_CLOSE_CODE,
...
describe('RealtimeClient connection lifecycle', () => {
  ...
  it('discards a stale token lookup after disconnect and creates only the new socket', async () => {
    ...
    // 첫 토큰이 준비되기 전에 연결을 끊고, 더 최신 세대의 연결을 새로 만든다.
    ...
    // 폐기된 연결 세대에서 뒤늦게 도착한 인증 정보가 두 번째 소켓을 만들면 안 된다.
  ...
  it('replays subscription recovery once on every authenticated socket', async () => {
    ...
    // 같은 소켓에서 준비 완료가 중복 발생해도 구독을 두 번 재전송하면 안 된다.
    sockets[0].receive('system.ready', ready('connection-1'))
    expect(recover).toHaveBeenCalledTimes(1)
    ...
    // 명시적으로 로그아웃하면 다음 인증 소켓을 만들기 전에 재연결용 등록 정보를 비워야 한다.
    client.disconnect()
  ...
  it('uses one forced HTTP refresh for a 4401 reconnect and then stops', async () => {
    ...
    // 강제 토큰 갱신 뒤에도 4401이 다시 발생하면 인증 반복을 막기 위해 최종 실패로 처리한다.
  ...
  it('abandons a stalled handshake and reconnects without rejecting connect()', async () => {
    ...
    // 만료 직전까지 연결을 유지한 뒤 기한을 넘겨 소켓 교체가 시작되는지 확인한다.
  ...
  it('refreshes at half of a short token TTL and deduplicates watcher refresh', async () => {
    ...
    // 토큰 수명의 절반 시점에 갱신하므로 마지막 1밀리초를 진행해 그 경계를 넘긴다.
    ...
    // 동시에 실행된 감시자는 새 요청을 보내지 않고 이미 예약된 갱신 작업을 재사용해야 한다.
