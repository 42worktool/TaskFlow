...
2. The receiver accepts or rejects it from the Friends page.
...
`/friends` is the dedicated authenticated page for:
...
Request received/accepted events are not pushed in this slice. The page reloads
on entry and WebSocket reconnection, and also provides a manual refresh button.
Blocking and request history remain outside the Toy scope.
