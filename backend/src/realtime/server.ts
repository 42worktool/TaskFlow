...
export class RealtimeServer {
  ...
  constructor(options: RealtimeServerOptions) {
    ...
    this.maxOutboundPayloadBytes =
      options.maxOutboundPayloadBytes ?? options.maxPayloadBytes;
    this.maxBufferedAmountBytes =
      options.maxBufferedAmountBytes ?? this.maxOutboundPayloadBytes * 4;
    this.shutdownDrainTimeoutMs =
      options.shutdownDrainTimeoutMs ?? DEFAULT_SHUTDOWN_DRAIN_TIMEOUT_MS;
    if (
      !Number.isSafeInteger(this.maxOutboundPayloadBytes) ||
      this.maxOutboundPayloadBytes <= 0
    ) {
    ...
    if (
      !Number.isSafeInteger(this.shutdownDrainTimeoutMs) ||
      this.shutdownDrainTimeoutMs <= 0
    ) {
  ...
  onConnectionDisconnected(
    ...
    const registration: RealtimeConnectionLifecycleListener<
  ...
  private async performClose(): Promise<void> {
    ...
    if (!acceptedTasksDrained || !lifecycleTasksDrained) {
      console.warn(
        `[realtime] shutdown drain exceeded ${this.shutdownDrainTimeoutMs}ms`,
      );
    }
    ...
    try {
      try {
        ...
        if (message.requestId) {
          this.sendRequestAcknowledgement(
            connection,
            message.event,
            result,
            message.requestId,
        ...
        const realtimeError =
          error instanceof RealtimeError
            ...
            : new RealtimeError(
                'HANDLER_FAILED',
                'The realtime event could not be handled',
                true,
              );
      ...
      if (
        ...
        !this.closing &&
        (connection.authenticationExpiryPending ||
          this.authenticationExpired(connection))
      ) {
        this.expireAuthentication(connection);
      }
    }
    ...
    try {
      ...
      if (!refresh) {
        ...
        const userConnectionIds =
          this.userConnections.get(principal.userId) ?? new Set<string>();
  ...
  private sendHandlerError(
    ...
    if (requestId) {
      ...
      if (
        fallbackResult !== 'not_serializable' &&
        fallbackResult !== 'too_large'
      ) {
        return;
      }
    }
  ...
  private assertCanJoin(connection: Connection): void {
    ...
    if (
      this.authenticationExpired(connection) &&
      connection.activeHandlerCount === 0
    ) {
      this.expireAuthentication(connection);
      throw new RealtimeError(
        'ACCESS_TOKEN_EXPIRED',
  ...
  private async drainTrackedTasks(deadline: number): Promise<boolean> {
    while (this.trackedTasks.size > 0) {
      ...
      if (
        !(await this.settleTrackedTaskSnapshot(
          [...this.trackedTasks],
          remainingMs,
        ))
      ) {
