...
// multer.StorageEngine doesn't publicly type `getFilename`, but DiskStorage
// exposes it — this lets us exercise the actual wired-in filename callback
// (encoding fix -> normalize -> length check) instead of just the pure
// helper functions it's built from.
...
test('fixOriginalnameEncoding recovers a Korean filename mangled by latin1 decoding', () => {
  ...
  // Simulates what busboy hands back: UTF-8 bytes mis-decoded as latin1.
...
test('requireMagicBytesMatch deletes and rejects an attachment binary disguised as a text file', async () => {
  // Contains a NUL byte, so it cannot be plain text.
