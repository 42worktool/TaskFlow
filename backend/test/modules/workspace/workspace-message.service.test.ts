// 워크스페이스 메시지의 권한과 저장, 기록 조회, 실시간 전달을 검증한다.
...
function message(id: string, content: string, createdAt: string, cardId: string | null = null) {
  return {
    id,
...
test('workspace messages require membership, return history, and deliver after create', async (t) => {
  ...
  const [{ prisma }, { realtime }, service] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
    import('../../../src/modules/workspace/workspace-message.service'),
  ])

  // 제한 개수만큼 최신순으로 조회한 뒤, 응답은 대화 흐름에 맞게 시간순으로 돌려준다.
  ...
  // 메시지 트랜잭션의 커밋이 확인된 뒤에만 실시간 이벤트를 전파한다.
  ...
  await t.test(
    'creates a selected-card comment atomically and delivers after commit',
    async (t) => {
      stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({
        role: 'VIEWER',
      }))
      ...
      stubMethod(t, prisma.workspaceMessage, 'create', async (args) => {
        ...
        messageCreate = args
        return message(FIRST_MESSAGE_ID, 'linked comment', '2026-07-29T00:00:01.000Z', CARD_ID)
      })
      ...
      // 하나의 트랜잭션에서 메시지와 댓글을 함께 저장하고, 커밋 뒤에 전달과 카드 무효화를 수행한다.
      const result = await service.createWorkspaceMessage({
        userId: USER_ID,
        workspaceId: WORKSPACE_ID,
      ...
      assert.deepEqual(deliveries, [[USER_ID, 'workspace.message_created', result]])
  ...
  await t.test('rejects a card outside the active workspace before writing', async (t) => {
    ...
    // 댓글 저장에 실패하면 함께 저장하던 메시지도 롤백하고 외부 이벤트는 발생시키지 않는다.
  ...
  await t.test('does not deliver or publish when the linked comment write fails', async (t) => {
    ...
    stubMethod(t, prisma.workspaceMessage, 'create', async () =>
      message(FIRST_MESSAGE_ID, 'linked comment', '2026-07-29T00:00:01.000Z', CARD_ID),
    )
    stubMethod(t, prisma.comment, 'create', async () => {
