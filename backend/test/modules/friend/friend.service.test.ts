// 친구 요청의 전체 생명주기와 친구 관계 상태, 실시간 알림을 검증한다.
...
test('friend service keeps pending requests separate from friendships', async (t) => {
  ...
  const [{ prisma }, { realtime }, friendService] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
    import('../../../src/modules/friend/friend.service'),
  ])

  // 조회 모델에서는 성립된 친구 관계와 방향성이 있는 친구 요청을 분리한다.
  ...
  // 친구 요청을 만들 때 사용자 쌍은 정규화하되 누가 요청했는지는 보존한다.
  ...
  await t.test('does not leave a request when acceptance wins a send race', async (t) => {
    ...
    // 두 번째 친구 관계 조회는 최초 확인 뒤 상대가 요청을 수락해 커밋된 상황을 재현한다.
    await assert.rejects(
      () =>
        friendService.sendFriendRequest({
          userId: USER_A,
  ...
  // 수락 시에는 받은 요청만 원자적으로 소비한 뒤 정규화된 친구 쌍을 생성한다.
  ...
  // 거절·요청 취소·친구 삭제는 각각 해당하는 정규화된 관계를 삭제하고 상대에게 알린다.
