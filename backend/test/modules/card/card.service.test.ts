// 카드 트랜잭션과 권한 재확인, 실시간 발행, 첨부파일 동작을 검증한다.
...
function stubMoveLocks(
  ...
  // 운영 코드와 같은 잠금 순서를 유지하기 위해 이동할 카드를 잠그기 전에 대상 목록을 잠근다.
...
test('createCard locks an active list before inserting the card', async (t) => {
  ...
  const [{ prisma }, { realtime }, { createCard }] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
    import('../../../src/modules/card/card.service'),
  ])
...
test('createCard rejects a list deleted before its row lock', async (t) => {
  ...
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
  stubMethod(t, prisma, '$transaction', async (operation) => operation(prisma))
  // 잠금 없는 사전 확인은 통과하지만, 최종 판단을 맡는 행 잠금 조회에서는 삭제 상태를 보게 한다.
...
test('updateCardCompletion toggles the card flag without moving lists', async (t) => {
  setRequiredEnvironment()
  const [{ prisma }, { realtime }, { updateCardCompletion }] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
    import('../../../src/modules/card/card.service'),
  ])
...
test('moveCard rejects a target list in another workspace', async (t) => {
  ...
  stubMethod(t, prisma.list, 'findFirst', async () => list(SOURCE_LIST_ID, SOURCE_WORKSPACE_ID))
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
  stubMoveLocks(t, prisma, { list_id: SOURCE_LIST_ID, user_id: null }, TARGET_WORKSPACE_ID)
  let findManyCalls = 0
  let updateCalls = 0
...
test('moveCard detaches stale workspace labels from an inbox card', async (t) => {
  ...
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
  stubMoveLocks(t, prisma, { list_id: null, user_id: USER_ID }, TARGET_WORKSPACE_ID)
  stubMethod(t, prisma.card, 'findMany', async () => [])
...
test('moveCard rejects viewers before mutating a card', async (t) => {
  ...
  stubMoveLocks(t, prisma, { list_id: SOURCE_LIST_ID, user_id: null }, SOURCE_WORKSPACE_ID)
...
test('moveCard rechecks inbox ownership after locking the card', async (t) => {
  ...
  stubMoveLocks(t, prisma, { list_id: null, user_id: TARGET_WORKSPACE_ID }, TARGET_WORKSPACE_ID)
...
test('getCard includes active labels and comments in its detail response', async (t) => {
  ...
  stubMethod(t, prisma.card, 'findFirst', async () => card({ list_id: null, user_id: USER_ID }))
...
test('card mutations use the locked location and publish only after commit', async (t) => {
  ...
  for (const mutation of mutationCases) {
    await t.test(mutation.name, async (t) => {
      // 잠금 → 권한 확인 → 저장 → 커밋 → 발행 순서를 보장하도록 각 경계를 기록한다.
      const operationOrder: string[] = []
      ...
      assert.ok(operationOrder.indexOf('permission') < operationOrder.indexOf('write'))
      assert.ok(operationOrder.indexOf('write') < operationOrder.indexOf('commit'))
      assert.ok(operationOrder.indexOf('commit') < operationOrder.indexOf('publish'))
...
test('updateCardDates validates against dates read under the card lock', async (t) => {
  ...
  stubMethod(t, prisma, '$transaction', async (operation) => operation(prisma))
  // 트랜잭션 전에 읽은 오래된 카드가 아니라, 잠근 행에서 읽은 마감일을 기준으로 검증해야 한다.
  stubMethod(t, prisma, '$queryRaw', async () => [
    {
      list_id: TARGET_LIST_ID,
      user_id: null,
  ...
  stubMethod(t, prisma.list, 'findFirst', async () => list(TARGET_LIST_ID, TARGET_WORKSPACE_ID))
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({
    role: 'MEMBER',
  }))
...
test('updateComment conditionally updates only an active comment', async (t) => {
  ...
  assert.equal(updated.comment_str, 'Updated')
...
test('updateComment rejects a delete race without publishing', async (t) => {
  setRequiredEnvironment()
  const [{ prisma }, { realtime }, { updateComment }] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
  ...
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
  stubMethod(t, prisma, '$transaction', async (operation) => operation(prisma))
  // updateMany 결과 0은 권한 확인과 조건부 수정 사이에 카드가 삭제된 상황을 나타낸다.
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof Error && 'code' in error && error.code === 'COMMENT_ALREADY_DELETED',
  )
  assert.equal(publishCount, 0)
})

test('deleteComment conditionally deletes only an active comment', async (t) => {
  setRequiredEnvironment()
  const [{ prisma }, { realtime }, { deleteComment }] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
  ...
  await deleteComment({ userId: USER_ID, commentId: COMMENT_ID })

  assert.deepEqual(deleteWhere, { id: COMMENT_ID, deleted_at: null })
  assert.equal(deleteData.deleted_by, USER_ID)
  assert.ok(deleteData.deleted_at instanceof Date)
  assert.equal(publishCount, 1)
})

test('deleteComment rejects a concurrent delete without publishing', async (t) => {
  setRequiredEnvironment()
  const [{ prisma }, { realtime }, { deleteComment }] = await Promise.all([
    import('../../../src/db'),
    import('../../../src/realtime'),
  ...
  stubMethod(t, prisma.list, 'findFirst', async () => list(SOURCE_LIST_ID, SOURCE_WORKSPACE_ID))
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
  // 조건부 소프트 삭제 결과 0은 다른 요청이 먼저 삭제를 완료한 상황을 나타낸다.
  stubMethod(t, prisma.comment, 'updateMany', async () => ({ count: 0 }))
  let publishCount = 0
  stubMethod(t, realtime, 'publish', () => {
    publishCount += 1
  ...
  await assert.rejects(
    () => deleteComment({ userId: USER_ID, commentId: COMMENT_ID }),
    (error: unknown) =>
      error instanceof Error && 'code' in error && error.code === 'COMMENT_ALREADY_DELETED',
  )
  assert.equal(publishCount, 0)
...
test('addAttachment stores the uploaded file metadata for a workspace member', async (t) => {
  ...
  stubMethod(t, prisma.card, 'findFirst', async () => card({ list_id: SOURCE_LIST_ID }))
  stubMethod(t, prisma.list, 'findFirst', async () => list(SOURCE_LIST_ID, SOURCE_WORKSPACE_ID))
  stubMethod(t, prisma.workspaceMember, 'findFirst', async () => ({ role: 'MEMBER' }))
...
test('removeAttachment soft-deletes the row', async (t) => {
  ...
  // 여기서는 실제 deleteUploadedFile()을 디스크에 없는 경로에 실행한다. fixture의
  // storage_key에 해당하는 파일을 저장한 적이 없으며, 함수가 내부에서 ENOENT를 무시하므로
  // 이 테스트는 소프트 삭제와 권한 경로만 확인한다. 파일 정리 자체는 lib/upload.test.ts에서 검증한다.
