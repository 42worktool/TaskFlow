-- 활동 로그 트리거의 적용 범위와 이벤트 중복 제거, 민감 정보 제거를 검증한다.
\set ON_ERROR_STOP on

-- 모든 Prisma 마이그레이션을 적용한 뒤 psql로 실행한다.
-- 전체 테스트를 트랜잭션으로 감싸 반복 실행할 수 있고 DB 상태가 남지 않게 한다.
BEGIN;
...
-- 다음 두 INSERT는 애플리케이션이 워크스페이스를 만들 때 수행하는
-- 소유자와 멤버의 중첩 저장 흐름을 재현한다.
...
-- 실제 변경이 없거나 감사 필드만 바뀐 경우에는 중복 UPDATE 로그를 만들면 안 된다.
UPDATE "Workspaces"
SET "name" = "name"
WHERE "id" = '10000000-0000-4000-8000-000000000001';
...
-- 하위 데이터를 채운 채로 두어 워크스페이스 소프트 삭제 시 하위 데이터는 보존하고
-- 워크스페이스 삭제 동작만 기록하는지 확인한다.
INSERT INTO "Cards" (
    "id", "list_id", "user_id", "title", "description", "sequence",
    "created_by", "updated_by", "created_at"
)
...
-- 소프트 삭제는 하위 데이터를 유지하며 선택한 집계 대상의 삭제 이벤트만 생성한다.
INSERT INTO "Workspaces" (
    "id", "name", "is_public", "created_by", "updated_by",
    "created_at", "updated_at"
)
