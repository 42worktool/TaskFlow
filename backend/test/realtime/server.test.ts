// 실시간 서버의 인증과 생명주기, 역압, 종료 규약을 검증한다.
...
import { RealtimeError } from '../../src/realtime/router'
import {
...
// 지연 가능한 gate로 긴 sleep 없이 생명주기 경쟁 상황을 결정적으로 재현한다.
...
describe('RealtimeServer', () => {
  ...
  it('removes every user connection from a channel and can clear the channel', async () => {
    httpServer = createServer()
    realtimeServer = new RealtimeServer(
      serverOptions((token) => validPrincipal(token === 'user-2-token' ? 'user-2' : 'user-1')),
    )
    realtimeServer.register(
      'workspace.subscribe',
      z.object({ workspaceId: z.string() }),
  ...
  it('expires an authenticated connection when its access token expires', async () => {
    httpServer = createServer()
    realtimeServer = new RealtimeServer(serverOptions(() => validPrincipal('user-1', 150)))
    realtimeServer.attach(httpServer)
  ...
  it('acknowledges an admitted handler before closing for token expiry', async () => {
    ...
    // 수락한 작업을 토큰 만료 뒤까지 보류해도 서버는 해당 응답을 먼저 마무리해야 한다.
  ...
  it('returns an admitted handler error before closing for token expiry', async () => {
    ...
    const started = deferred()
    const release = deferred()
    realtimeServer = new RealtimeServer(serverOptions(() => validPrincipal('user-1', 120)))
    realtimeServer.register('example.delayed-failure', z.object({}), async () => {
      started.resolve()
      await release.promise
    ...
    // 만료 전에 수락한 핸들러가 실패해도 요청 식별 정보는 유지되어야 한다.
    await started.promise
    await delay(180)
    assert.equal(socket.readyState, WebSocket.OPEN)
  ...
  it('emits authenticated and disconnected lifecycle snapshots', async () => {
    ...
    assert.equal(
      authenticatedInfo.connectionId,
      (ready.data as { connectionId: string }).connectionId,
    )
  ...
  it('drains an admitted handler before closing realtime dependencies', async () => {
    ...
    const release = deferred()
    realtimeServer = new RealtimeServer(serverOptions(() => validPrincipal()))
    realtimeServer.register('example.shutdown-work', z.object({}), async () => {
      started.resolve()
      await release.promise
    ...
    // 작업이 대기 중일 때 종료를 시작해 연결 닫기가 이미 수락한 핸들러를 기다리는지 확인한다.
  ...
  it('bounds shutdown drain time when an accepted handler does not settle', async () => {
    ...
    realtimeServer.register('example.stuck-shutdown-work', z.object({}), async () => {
      started.resolve()
      await release.promise
      finished.resolve()
    })
    ...
    // 핸들러가 계속 대기하더라도 시간 제한으로 의존성 종료의 최대 대기 시간을 제한한다.
  ...
  it('orders and drains async lifecycle work for each connection', async () => {
    ...
    // 서버 종료 중에도 이 소켓에서는 인증 처리가 연결 해제보다 먼저 끝나야 한다.
    let closeResolved = false
    const realtimeClosed = realtimeServer.close().then(() => {
      closeResolved = true
    })
  ...
  it('does not let a delayed handler join a channel after disconnect', async () => {
    ...
    // 이벤트 전달 뒤 처리를 멈추고 연결을 끊은 다음 재개해 오래된 컨텍스트 보호 로직을 검증한다.
  ...
  it('returns correlated errors for oversized and non-serializable responses', async () => {
    ...
    realtimeServer.register('example.large-error', z.object({}), () => {
      throw new RealtimeError('X'.repeat(1_000), 'Y'.repeat(1_000), true)
    })
    realtimeServer.attach(httpServer)
    ...
    // 잘못된 응답은 해당 요청만 실패시켜야 하며 소켓은 계속 사용할 수 있어야 한다.
  ...
  it('closes with resync required when context send cannot serialize data', async () => {
    httpServer = createServer()
    realtimeServer = new RealtimeServer(serverOptions(() => validPrincipal()))
    realtimeServer.register('example.push-cyclic', z.object({}), (context) => {
      const value: { self?: unknown } = {}
      value.self = value
